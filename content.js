...
function macrostart() {
	...
	if (coachSelected.length == 0 && firstSelected.length == 0 && waitingSelected.length == 0) {
		...
		// location.reload()를 버튼클릭으로 변경
		let btn_tst = document.querySelector('input.inquery_btn');
		btn_tst.click();
		...
		if (sessionStorage.getItem('macro') == "true") {
			...
			if ($("#search-list").length != 0) {
				...
				if (succeed == true) {
					...
					chrome.runtime.sendMessage({ type: "DISCORD_ALERT" });
					...
					//새로고침시 목록 안뜨는 경우 있음. 조회버튼 누르기로 변경.
					//location.reload();	
					
					let btn_tst = document.querySelector('input.inquery_btn');
					btn_tst.click();
					
					}, 1000+( Math.floor( Math.random()*500 ) ));	//1000+a (ms) 딜레이. 기본값 1000
				}
			} else {
				history.go(-1);
			}
